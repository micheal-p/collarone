...
- [x] **36. (founder-reported, fixed 2026-08-08) MEDIUM ux** — Landing page — mobile menu
  ...
  SHIPPED: scrim (tap anywhere to close) + Escape, plus keys on every AnimatePresence child (Landing menu, ChatWidget launcher and panel) guarded by test/animatepresence_keys.mjs.
  ⚠ NOT LIVE-VERIFIED AT MOBILE WIDTH. macOS Chrome in this environment refuses to resize below 1440 CSS px, so the <=780px breakpoint was never actually reached. Testing at 1440px showed the menu node lingering in the DOM after close, and that was first written up as a keyless-AnimatePresence root cause — but that test is INVALID: at 1440px .cl-mobile-menu is display:none, and browsers do not run animations on hidden elements, so Framer's exit-completion callback cannot fire and the node stays regardless of keys. The observed "never unmounts" is therefore an artifact of the test, not evidence about phones. The keys are correct practice and harmless either way. OUTSTANDING: someone must open collarone.app on a real phone, open the menu, and tap off it. If it closes, this item is genuinely done; if not, the real cause is still unknown.
