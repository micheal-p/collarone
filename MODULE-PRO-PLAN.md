...
| The automation cron endpoint is unauthenticated | **FALSE in production — the code pattern was genuinely fail-open, but the secret was set** | `automations-run.js` did read `if (secret && ...)`, which skips the check entirely when `CRON_SECRET` is unset — a real fail-open pattern, now fixed to refuse (503) instead. But the endpoint was NOT exposed: an unauthenticated POST to production answers **401**, which under the new code can only happen when the secret exists. `CRON_SECRET` is set on the box; it simply isn't in `deploy/` or `.github/`, which is where I looked. **My error: I inferred a runtime environment variable's absence from its absence in the repo. Environment variables live on the server. The cheap safe check — asking, or reading the response code — was skipped.** The hardening is still worth keeping as defence in depth. |
