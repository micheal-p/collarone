...
function priceQuote(text) {
  ...
  const staffNote = (staff ? '' : `\nEvery plan adds ${N(PER_STAFF)} per staff member per month.`) + `\nYou don\x27t pick a plan, you pick your suites and we put you on the cheapest plan for them, so you can never overpay. The more suites you run, the less each one costs.`;
...
const INTENTS = [
  ...
  },
  {
    ...
    answer: 'Payroll runs the 2026 Nigeria Tax Act rules: the new PAYE bands (0% up to ₦800,000, then 15% to 25%), rent relief in place of the old CRA, plus Pension, NHF and NSITF, with payslips, staff loans and salary advances repaid by automatic deduction, and a Bank payments queue for whoever liaises with your bank. Important honesty: Collarone never touches your bank account. It prepares the disbursement instruction; your bank executes it. Payroll is available to Nigerian-registered companies.',
  ...
  },
  {
    ...
    answer: 'Your per-seat and tier rates lock in at sign-up, so later published price changes don\'t move your bill. And everything is priced in naira with no forex markup, so your cost doesn\'t swing with the exchange rate.',
    chips: ['What does it cost?', 'How do I get started?'],
  },
  {
    id: 'leave',
