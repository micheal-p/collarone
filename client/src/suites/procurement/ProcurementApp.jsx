  ...
  return (
    <div className="modal-overlay" onMouseDown={onClose}>
      <div className="modal modal-wide" onMouseDown={(e) => e.stopPropagation()}>
        <div className="modal-head"><h2>Add vendor</h2></div>
...
function RequestModal({ vendors, onClose, onSaved, flash }) {
  ...
  return (
    <div className="modal-overlay" onMouseDown={onClose}>
      <div className="modal modal-wide" onMouseDown={(e) => e.stopPropagation()}>
        <div className="modal-head"><h2>New purchase request</h2></div>
        <form className="modal-body" onSubmit={submit}>
      ...
      {!loading && tab === 'requests' && (
        <div className="table-wrap">
          <table className="table">
            <thead><tr><th>Item</th><th>Vendor</th><th>Requested by</th><th>Total</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {requests.length === 0 && <tr><td colSpan={6} className="td-empty">No purchase requests yet.</td></tr>}
              {requests.map((r) => (
                <tr key={r.id}>
      ...
      {!loading && tab === 'vendors' && (
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              {vendors.length === 0 && <tr><td colSpan={isManager ? 5 : 4} className="td-empty">No vendors yet.</td></tr>}
              {vendors.map((v) => (
                <tr key={v.id}>
