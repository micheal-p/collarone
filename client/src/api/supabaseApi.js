...
export async function supabaseApi(path, opts = {}) {
  ...
  if (method === 'PATCH' && seg[0] === 'payroll' && seg[1] === 'lines' && seg.length === 3) {
    const patch = {};
    // Writes MANUAL deductions only. other_deductions used to hold the manual
    // amount AND the loan instalment the system had baked in, so typing ₦5,000
    // for a uniform silently wiped a ₦20,000 loan repayment — overpaying the
    // employee while payroll_line_loans still claimed it was collected. The
    // database now derives other_deductions and net from the two parts, so
    // neither this route nor anything else can desync them.
    if (body.otherDeductions !== undefined) patch.manual_deductions = Number(body.otherDeductions) || 0;
