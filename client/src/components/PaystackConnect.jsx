...
export default function PaystackConnect({ flash, isAdmin, nudge = false }) {
  ...
  // true = stored key answered Paystack just now; false = Paystack rejected
  // it (rotated/revoked in their dashboard); null = couldn't check.
  ...
  const load = () => call({ action: 'status' }).then((d) => { setEnabled(d.enabled); setPublicKey(d.publicKey || ''); setVerified(d.verified ?? null); }).catch(() => {}).finally(() => setLoading(false));
  ...
  // A nudge surface stays quiet once payments work — but a key Paystack now
  // REJECTS is exactly what the money path must shout about.
  if (nudge && enabled && verified !== false) return null;

  const brokenKey = enabled && verified === false;
  ...
  return (
    <div style={{ border: '1px solid var(--line)', borderRadius: 10, padding: '14px 16px', margin: '16px 0', background: 'var(--surface-2)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, borderRadius: 100, padding: '1px 9px', background: enabled && !brokenKey ? '#dff6dd' : 'var(--surface)', color: enabled && !brokenKey ? '#12833F' : 'var(--text-2)', border: enabled && !brokenKey ? 'none' : '1px solid var(--line)' }}>{enabled ? (brokenKey ? 'Needs attention' : 'ON') : 'Off'}</span>
        {enabled && verified === true && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, color: '#12833F' }}>
      ...
      {isAdmin && open ? (
        ...
        <>
          <p className="muted" style={{ fontSize: 12.5, margin: '0 0 10px', lineHeight: 1.6 }}>
            Paystack is rejecting your saved key — it was likely rotated or revoked in your Paystack dashboard. Card payments on your store and invoices are failing until it&rsquo;s reconnected.
          </p>
          {isAdmin ? (
            <button type="button" className="btn btn-primary btn-sm" onClick={() => setOpen(true)}>Reconnect Paystack</button>
          ) : (
            <p className="muted" style={{ fontSize: 12.5, margin: 0 }}>Ask your workspace admin to reconnect it.</p>
          )}
        </>
      ) : enabled ? (
        <>
